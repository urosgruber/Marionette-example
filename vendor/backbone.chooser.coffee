do (Backbone) ->
    ...
    class BaseChooser
        ...
        _publicMethods: -> ["choose", "unchoose", "getChosen", "getFirstChosen", "chooseById", "isAllChosen"]
        ...
        isAllChosen: ->
            _.size(@chosen) is @collection.length
    ...
    class Backbone.SingleChooser extends BaseChooser
        ...
        unchoose: (model, options) ->
            return if not @modelInChosen(model)
    ...
    class Backbone.MultiChooser extends BaseChooser
        ...
        unchoose: (args...) ->
            ...
            for model in _([args]).flatten().value()
                ## break if the model is already chosen
                break if not @modelInChosen(model)
